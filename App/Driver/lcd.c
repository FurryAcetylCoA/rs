...
{
    __HAL_TIM_SET_COUNTER(&htim4,0); // 将装载值计0
    HAL_TIM_Base_Start(&htim4); // 开始计数
    while (__HAL_TIM_GetCounter(&htim4) < num); //当装载值小于预设值时循环
    HAL_TIM_Base_Stop(&htim4); //结束延时
}
